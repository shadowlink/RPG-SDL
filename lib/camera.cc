...
{
	Uint32 centroX=0, centroY=0, inicial[2], lim_right[2], lim_bottom[2], id=0, x=0, y=0, i=0, j=0;
	...
	for(i=inicial[0]; i<lim_bottom[1]+1; i++)
	{
		for(j=inicial[1]; j<lim_right[0]+1; j++)
		{
	...
	heroe->Dibujar(screen, scrollX, scrollY);

	//Elementos traspasables por detras
	for(i=inicial[0]; i<lim_bottom[1]; i++)
	{
		for(j=inicial[1]; j<lim_right[0]; j++)
		{
			strcpy(buffer, mapa->getSueloTras(i, j).getTileId().c_str());
			id=atoi(buffer)-1;
			img->dibujar(screen, id ,(32*j)-scrollX, (32*i)-scrollY);
		}
	}
